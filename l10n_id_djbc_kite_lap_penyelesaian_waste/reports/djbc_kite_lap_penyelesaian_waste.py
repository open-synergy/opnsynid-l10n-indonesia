...
class LapKitePenyelesaianWaste(models.Model):
    _name = "l10n_id.lap_kite_penyelesaian_waste"
    _description = "Laporan Penyelesaian Waste Untuk KITE"
    _auto = False

    nomor = fields.Many2one(
        string="Nomor",
    ...
    def _get_movement_type(self, cr):
        query = """
            ...
            FROM  ir_model_data
            WHERE
            module = 'l10n_id_djbc_kite_lap_penyelesaian_waste' AND
            name = 'djbc_kite_movement_type_penyelesaian_waste'
        """
        cr.execute(query)
        ...
        join_str = """
            ...
            JOIN (
                ...
                WHERE
                    ...
                    (e1.name = 'product_categ_kite_scrap')
                ) as h ON
                g.id = h.res_id
        """
        return join_str
